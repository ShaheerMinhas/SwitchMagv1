...
The project's methodology revolves around an innovative, algorithm-based approach using an LSTM classifier, focusing on enhancing Pakistani Sign Language (PSL) recognition.
...
## Data Collection and Frame Processing:

The process begins with collecting a dataset comprising PSL videos. This dataset undergoes a two-step processing:

1. Frame Extraction: Individual frames are extracted from each video to capture specific
sign language gestures.

2. Pre-Processing: These frames are then pre-processed to remove background noise,
facilitating clearer landmark detection.

## Landmark Detection and Classification:

The selected landmark detection algorithm is applied to the processed frames to identify key
hand points, which are essential for tracking hand movements.

1. Frame Storage: The frames with detected landmarks are stored for further analysis.

2. Classification with LSTM: These frames are classified using an LSTM classifier, which
involves a two-phase process of testing (with known labels) and training (using the testing dataset) to evaluate the model's accuracy.

## Accuracy Enhancement and Correction:

The project places significant emphasis on improving recognition accuracy.

1. Manual Correction: Identified landmarks are manually reviewed and corrected for any
misidentifications caused by the initial algorithms.

2. Re-evaluation: Post-correction, the model is re-trained and tested. This phase involves
comparing the accuracies pre- and post-correction to gauge improvements.

## Real-Time Application Development:

As a final step, the project aims to extend the refined model for real-time sign language
recognition, making it applicable on devices equipped with webcams. This extension is pivotal for practical, real-world applications, enhancing the accessibility and utility of the developed system.

# Challenges and Solutions

The project faced significant challenges, particularly in the accuracy of landmark detection. The primary algorithms used, MediaPipe and OpenPose, occasionally misidentified hand landmarks due to factors like rapid movement, background interference, or poor video quality. MediaPipe sometimes misplaced landmarks when the fingers were not clearly visible or moving quickly, leading to incorrect interpretations of hand gestures. Similarly, OpenPose's performance was hindered by its computational intensity, especially with high-resolution videos, and its setup was complex and not well-documented, posing difficulties for efficient implementation. To overcome these obstacles, the team employed a meticulous process of manual correction. This involved closely examining each frame to identify and rectify any inaccuracies in landmark detection. By combining the computational power of the algorithms with human precision, the significantly enhanced the accuracy of PSL recognition. This dual approach of leveraging advanced technology alongside detailed manual review proved crucial in addressing the challenges, ultimately leading to a more reliable and effective recognition system.

# Results and Analysis

The project's primary achievement was a substantial improvement in the accuracy of the
landmark detection model for Pakistani Sign Language (PSL). Initially, the model's accuracy
was around 40%, but after implementing the landmark correction process, a significant jump to approximately 70% was observed. This marked improvement highlighted the effectiveness of the correction process. An interesting discovery during the project was the impact of omitting the 'z' landmark from consideration. By focusing solely on the 'x' and 'y' coordinates, the model's accuracy further improved, demonstrating that certain landmarks had more influence on the overall performance. However, the results varied when employing different algorithms. For instance, the OpenPose algorithm showed only minimal improvement after the correction process. This was attributed to its relative newness in the field of landmark detection and its tendency to detect broader pose key points, which made it less efficient for the specific task of hand landmark detection in sign
language. These findings underscore the project's success in enhancing the accuracy of PSL recognition.
Looking ahead, the team envisions applying this landmark correction methodology to other
datasets and algorithms, expanding the scope and impact of the project in the field of sign
